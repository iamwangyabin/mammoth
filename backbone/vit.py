...
class ViT(MammothBackbone):
    def __init__(self, num_classes: int) -> None:
        ...
        self.classifier = nn.Linear(self.image_encoder.embed_dim, num_classes)
        self.image_encoder.head=None

    def forward(self, x: torch.Tensor, returnt='out') -> torch.Tensor:
        feature = self.image_encoder(x)["features"]
        if returnt == 'features':
            return feature
...
def vit_base_16(nclasses: int, nf: int = 64) -> ViT:
    return ViT(nclasses)
